...
class MarkdownReportFormatterTest {
    ...
    @Test
    void formatsCategorySectionsWithLinks() {
        ...
        CategoryTrendSection section = new CategoryTrendSection(
        ...
        TrendReport report = formatter.format(List.of(section));

        assertThat(report.markdown()).contains("[2026-04-06 최신 트렌드 리포트]");
        assertThat(report.markdown()).contains("## 1. 분야별 핵심 요약");
        assertThat(report.markdown()).contains("## 2. 분야별 상세 리포트 및 중요 뉴스");
        assertThat(report.markdown()).contains("### 개발");
        assertThat(report.markdown()).contains("GitHub ships new Copilot workflow tooling");
        assertThat(report.markdown()).contains("워크플로 자동화가 팀 생산성과 협업 방식에 직접 영향을 줍니다.");
    }
}
